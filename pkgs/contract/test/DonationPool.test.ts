...
describe("DonationPool", async () => {
  ...
  it("Should not be able to donate 0", async () => {
    // create targetToken
    const token = await viem.deployContract("ExampleToken");
    // deploy the DonationPool contract
    const donationPool = await viem.deployContract("DonationPool", [
  ...
  it("Should emit the donated event when calling the donate() function", async () => {
    // create targetToken
    const token = await viem.deployContract("ExampleToken");
    // deploy the DonationPool contract
    const donationPool = await viem.deployContract("DonationPool", [
    ...
    assert.equal((events[0].args as any).donor.toLowerCase(), donor1.account.address.toLowerCase());
    assert.equal((events[0].args as any).amount, donationAmount);
  });

  it("Should emit EtherDonated event when ETH is sent directly", async () => {
